#!/bin/tcsh
#SBATCH --output=/home/bz5265/wave/SHiELD_run/DP_TC_WAVE/stdout/%x.o%j
#SBATCH --job-name=TC_DP_WAVE
...
set BUILD_DIR = "~${USER}/wave/SHiELD_build/"
...
set warm_start = ".false."                          # if true, set restart_dir
set res = "2km"                                     # available options: 1. 1km;  2. 2km; 3. 4km
set fullcoupling = ".true."                         # If .true., two-way SHiELD-MOM6 coupling is activated; otherwise .false., one-way is used. 
                                                    # For one-way coupling, variables from SHiELD are used to force MOM6, but SHiELD doesn't receive feedback from ocean.
set wavecoupling = ".true."                        # If .true., SHiELD-MOM6-WW3 coupled mode; .false., SHiELD-MOM6 coupled mode              

if ("${fullcoupling}" == ".false.") then
   set MEMO = ${hord_option}"_"${deglat}"N_"${sst}"K_"${ini_storm}"_"${days}"d_"${res}"_one_way"
else if ("${fullcoupling}" == ".true.") then
   set MEMO = ${hord_option}"_"${deglat}"N_"${sst}"K_"${ini_storm}"_"${days}"d_"${res}"_two_way_newcode"
else
   echo "Error: Invalid value for fullcoupling. Expected .fale. or .true., but got '${fullcoupling}'. Exiting..."
   exit 1
endif

if ("${wavecoupling}" == ".false.") then
# If run SHiELD or SHiELD-MOM6 coupled mode, options: 'hwrf17','coare3.5','beljaars', or 'charnock'
   set CPL_DIR = "NOWAVE"
   set rough_scheme = "'hwrf17'"
   set do_iter_monin_obukhov = ".true." 
else if ("${wavecoupling}" == ".true.") then
# If run SHiELD-MOM6-WW3  coupled mode, rough_scheme should be set as 'wblm'. In this case, the roughness from WW3 will be used.
   set CPL_DIR="WAVE"
   set rough_scheme = "'wblm'"
   set do_iter_monin_obukhov = ".false."
else
   echo "Error: Invalid value for wavecoupling. Expected .true. or .false., but got '${wavecoupling}'. Exiting..."
   exit 1
endif
...
# directory structure
set WORKDIR    = ${BASE_DIR}/${RELEASE}/${CPL_DIR}/${DATE}.${CASE}.${TYPE}.${MODE}.${MONO}.${MEMO}/
set executable = ${BUILD_DIR}/Build/bin/${RELEASE}_${TYPE}.${COMP}.${MODE}.${EXE}
...
if (${wavecoupling} == ".true.") then
   ln -sf /home/bz5265/ww3_proc/ww3_run/mod_def.ww3 .
   ln -sf /scratch/cimes/mouallem/For_biao_from_cheng/cases/INPUT_2km/* INPUT/
else if (${wavecoupling} == ".false.") then
   ln -sf /scratch/cimes/mouallem/from_gaea/shiemom_pdata/INPUT/DP_2km/* INPUT/
else
   echo "Error: Invalid value for wavecoupling. Expected .false. or .true., but got '${wavecoupling}'. Exiting..." 
   exit 1
endif
...
/
 ...
 &surface_flux_nml
       ...
       raoult_sat_vap = .true.
       do_iter_monin_obukhov = ${do_iter_monin_obukhov} ! KGao: turn this on for hwrf17 rough scheme
...
/
 ...
 &atmos_model_nml
       ...
       fdiag = $fdiag
       fullcoupler_fluxes = ${fullcoupling}
       debug = .false.
/
...
/
 ...
 &coupler_nml
       ...
       seconds = $seconds
       dt_atmos = $dt_atmos
       do_ocean=.true.               !off with shield, on with shieldfull
       do_ice=.true.
       do_waves= ${wavecoupling}             
       do_land=.false.             
       do_atmos=.true.           
       do_flux=.true.           
       dt_cpld = $dt_cpld        !off with shield, on with shieldfull
...
\$ -------------------------------------------------------------------- \$
  ...
  'input' F F F F F F F
\$  'ww3'
  'ww3'  'no' 'CPL:native' 'CPL:native' 'no' 'no' 'no' 'no'   1  1  0.00 1.00  F
   ${start_yyyymmdd} ${start_hhmmss}   ${end_yyyymmdd} ${end_hhmmss}
  F F
   ${start_yyyymmdd} ${start_hhmmss}   3600 ${end_yyyymmdd} ${end_hhmmss}
  N
  DPT CUR WND HS UST CHA T01 TAW TWA DIR T0M1 FP DP PHS PTP PDIR USP
   ${start_yyyymmdd} ${start_hhmmss}   0 ${end_yyyymmdd} ${end_hhmmss}
...
\$  separated wave field data
   ${start_yyyymmdd} ${start_hhmmss}      0  ${end_yyyymmdd} ${end_hhmmss}
  'the_end'  0
  'MOV' ${start_yyyymmdd} ${start_hhmmss}   5.  90.
  'STP'
